...
function wpse_110037_new_posts($post_id){
    ...
    if($post_type == 'product' && $deals_of_the_day[0] == '1') {
			...
			//~ update_post_meta($post_id,'deal_of_day_price', $actual_price);
...
function custom_price_html( $price, $product ){	
	...
	$deal_of_day_price = get_post_meta( $product->id, 'deal_of_day_price', true );
	
	
	if(is_page( array( 'deals-of-the-day'))) {
		if ( ! empty( $deal_of_day_price ) ) {
			$price = '<span class="amount"> ' . wc_price( $deal_of_day_price ) . ' per kg</span>';	
		}else{	
			return str_replace( '</ins>', ' / Piece </ins>', $price );
		}
	}else{
